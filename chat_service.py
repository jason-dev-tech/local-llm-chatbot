...
def build_route_metadata(
    ...
    *,
    ...
    normalized_route = route if route in {"chat", "rag", "tool"} else "chat"
    ...
    return {
        ...
        "source_count": len(extract_source_list(chunks)) if normalized_route == "rag" else None,
    }
...
def build_rag_messages(session_id, user_input):
    ...
    if context_text:
        system_prompt = (
            ...
            "When multiple sources are relevant, synthesize them into one answer instead of treating each source separately.\n"
            "Compare, summarize, or reconcile information across documents when the user's question calls for it.\n"
...
def build_retrieval_summary_request(user_input, session_id=None):
    ...
    summary_prompt = (
        ...
        "- Combine information from multiple retrieved documents when more than one source is relevant.\n"
        "- Compare or summarize across documents when the request asks for cross-document context.\n"
...
def send_message_and_stream(session_id, user_input, metadata_callback=None):
    ...
    try:
        ...
        if tool_workflow_result is not None:
            ...
            route_reason = tool_workflow_result.reason
            route_confidence = tool_workflow_result.confidence
            log_observability_event(
                "route",
                session_id=session_id,
